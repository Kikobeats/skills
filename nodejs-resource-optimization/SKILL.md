---
name: nodejs-resource-optimization
description: Identify, validate, and ship production-safe performance and resource-usage fixes in Node.js codebases. Use when users ask to reduce CPU, memory, event-loop lag, file descriptor pressure, network/socket overhead, queue contention, timeout/retry amplification, or to run one-PR-per-improvement optimization workflows with benchmarks.
---

# Node.js Resource Optimization

Use this workflow to turn Node.js performance/resource investigations into safe, reviewable PRs.

## Goals

- Improve throughput and tail latency without changing intended behavior.
- Reduce CPU, memory, event-loop lag, I/O pressure, or lock contention in hot paths.
...
- Include both latency and resource deltas when applicable.
...
5. Benchmark before/after numbers.
6. Risk assessment.
